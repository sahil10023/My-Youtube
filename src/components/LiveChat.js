...
const LiveChat = () => {
    ...
    useEffect(() => {
        const intervel = setInterval(() => {
            dispatch(addMessage({ name: generateRandomnames(), message: generateRandomMessage() }));
        }, 700);
        return () => clearInterval(intervel);  // cleanup function to stop the interval when component unmounts
    });

    return (<>

        <div className='ml-2 w-full h-[450px] p-2 overflow-y-auto flex-col-reverse flex  border border-black bg-slate-100 rounded-lg'>
